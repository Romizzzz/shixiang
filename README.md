...
- 选择矩形、正方形、圆形或椭圆。
- 点击开始截图后，主窗口隐藏；按住鼠标左键拖动，松开完成。
- 右键或 Esc 取消并返回主窗口。
- 可设置输出宽高（px）。圆形和正方形要求宽高相同。
- 自定义尺寸是截图后的重采样，不是屏幕选区尺寸；非等比例设置会拉伸图像。
...
已通过 7 项核心测试及 Windows 桌面流程测试，覆盖隐藏窗口、选区裁切、
自定义输出像素、右键取消和压缩。混合 DPI 多屏场景尚未实机验证。
